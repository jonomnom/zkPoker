...
template Poseidon104() {
    signal input inputs[104];
    signal output final_out;
    signal out[9];
    component poseidons[8];
    out[0] <== 0;
    for (var i = 0; i < 8; i++) {
        poseidons[i] = Poseidon(14);
        for (var j = 0; j < 13; j++) {
            poseidons[i].inputs[j] <== inputs[i*13 + j];
        }
        poseidons[i].inputs[13] <== out[i];
        out[i+1] <== poseidons[i].out;
    }
    final_out <== out[8];
}
